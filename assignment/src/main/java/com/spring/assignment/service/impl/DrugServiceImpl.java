	...
	public ApiResponse searchBySubstanceName(String substanceName, int pageNumber, int recordSize) {
		if (substanceName != null && !substanceName.isBlank()) {
			try {
				...
				String url = String.format(Constants.FDA_BASE_URL, substanceName, Constants.SUBMISSION_STATUS,
						recordSize);
				...
				finalResult = results.stream().map(drugRecord -> {
					DrugRecord drugRecordObj = DrugRecord.builder()
							...
							.substanceName(Arrays.asList(substanceName))
							.sponsorName((String) drugRecord.get("sponsor_name")).build();
				...
				if (e.getStatusCode() == HttpStatus.NOT_FOUND) {
					throw new ResourceNotFoundException("FDA API returned 404 for " + substanceName);
				}
				throw e;
			}
		} else {
			throw new RuntimeException("Substance Name should be not empty");
		}
	}
